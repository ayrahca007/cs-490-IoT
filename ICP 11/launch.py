...
def home():
    ...
    return render_template("index.html", form=form, result="uploadimage.jpg")

@app.route("/about")
def about():
    return "ICP for Object detection from image"
...
net = cv2.dnn.readNetFromCaffe(os.path.join(os.getcwd(), "detect", "MobileNetSSD_deploy.prototxt.txt"),
    os.path.join(os.getcwd(), "detect", "MobileNetSSD_deploy.caffemodel"))

def obj_detect(path):
    image = cv2.imread(path)
    (h, w) = image.shape[:2]
    blob = cv2.dnn.blobFromImage(cv2.resize(image, (300, 300)), 0.007843, (300, 300), 127.5)
    print("[INFO] Object detection in process...")
    net.setInput(blob)
